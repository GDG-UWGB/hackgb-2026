    ...
    return (
        <section className="relative pt-20 pb-32 px-4 overflow-hidden" id="sponsors">
            ...
            <div className="max-w-7xl mx-auto text-center relative z-10">
                ...
                <motion.div
                    {...fadeUp(0.15)}
                    className="bg-white/45 backdrop-blur-xl rounded-2xl border border-white/25 shadow-xl overflow-hidden flex flex-col min-h-[500px] relative text-left"
                >
                    {/* IDE Top Window Bar */}
                    ...
                    <div className="flex border-b border-black/5 bg-white/20 overflow-x-auto scrollbar-none">
                        <div className="flex items-center gap-2 px-6 py-3 border-r border-black/5 font-google-mono text-xs font-medium bg-white/60 text-[#0C3C34] border-t-2 border-t-[#61A644] flex-1 justify-center">
                    ...
                    <div className="p-8 md:p-12 bg-transparent space-y-14">
                        {/* Phoenix Tier */}
                        {phoenixSponsors.length > 0 && (
                            <div className="text-center">
                                ...
                                {renderSponsorCards(phoenixSponsors, 'w-64 h-32', 'h-16 md:h-20')}
                            </div>
                        )}
                        ...
                        {flameSponsors.length > 0 && (
                            <div className="text-center">
                                ...
                                {renderSponsorCards(flameSponsors, 'w-56 h-28', 'h-14 md:h-16')}
                            </div>
                        )}
                        ...
                        {emberSponsors.length > 0 && (
                            <div className="text-center">
                                ...
                                {renderSponsorCards(emberSponsors, 'w-48 h-24', 'h-12')}
                            </div>
                        )}
                                ...
                                <div className="flex flex-wrap gap-6 justify-center items-center">
                                    ...
                                    {sparkSponsors.map((sponsor, idx) => (
                                        <a key={sponsor.name} href={sponsor.link} target="_blank" rel="noopener noreferrer">
                                            <motion.div
                                                {...fadeUp(0.05 * (idx + 1))}
                                                className="group bg-white border border-black/5 rounded-xl px-6 py-4 flex items-center justify-center gap-3 w-44 h-20 hover:border-slate-350 hover:shadow-sm transition-all duration-300"
                                            >
                                                <img
                                                    src={sponsor.logo}
                                                    alt={sponsor.name}
                                    ...
                                    <motion.a
                                        ...
                                        className="group rounded-xl px-6 h-20 border border-dashed border-[#61A644]/40 hover:border-[#61A644]/70 bg-white hover:bg-white/90 transition-all duration-300 flex flex-col items-center justify-center gap-1 w-44 cursor-pointer"
                                    >
                                        <ArrowUpRight className="w-4.5 h-4.5 text-[#61A644]/60 group-hover:text-[#61A644] transition-colors" />
                                        <span className="font-google font-bold text-xs text-[#61A644]/80 group-hover:text-[#61A644] transition-colors">
                                            Become Sponsor
                                        </span>
                                    </motion.a>
                                </div>
